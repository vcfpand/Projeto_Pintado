...
def load_data():
    try:
        ...
        # PROTEÇÃO CONTRA DUPLICAÇÃO DE BIOMETRIA
...
if df is not None:
    ...
    # ==========================================
    # CARDS DE DESEMPENHO (KPIs Corrigidos)
    # ==========================================
    ...
    for i, trat in enumerate(trat_sel):
        ...
        # Médias de água mantêm-se brutas (capturam flutuações de manhã e tarde)
        m_ph = d_trat['ph'].mean()
        ...
        # O consumo e mortalidade procuram apenas o último dia isolado!
        ...
        # O acumulado é a soma do consumo máximo registado por caixa até o dia do filtro
        ...
        dados_gemini[trat] = {"Consumo_Diário": cons_hoje, "Var_%": delta_cons, "Mort_Acumulada": mort_total, "Amonia_Media": m_amonia, "OD_Medio": m_od}
        ...
        with cols[i]:
            with st.container(border=True):
                ...
                st.metric("Consumo do Último Dia", f"{cons_hoje:.0f} g", f"{delta_cons:.1f}% vs Ontem", delta_color="normal")
                st.divider()
